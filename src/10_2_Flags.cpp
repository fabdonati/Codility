...
int solution(std::vector<int> &A) {
    std::vector<int> peaksDistance;
    int lastPeak = 0;

    for (int i = 1; i < A.size()-1; i++) {
        if (A[i] > A[i-1] && A[i] > A[i+1]) {
            if (!peaksDistance.empty()) {
                peaksDistance.push_back(i - peaksDistance.back());
            } else {
                peaksDistance.push_back(i);
            }
        }
    }

    int nFlags = peaksDistance.size();
    for (auto const &p : peaksDistance) {
        if 
    }

    while (i > 0) {
        if (std::all_of(deltas.begin(), deltas.end(), [&i](int j){
            if (j>=i) counter++;
            return counter ;
            })) {
            return i;
...
int main() {
    std::vector<int> test{1,5,3,4,3,4,1,2,3,4,6,2};
